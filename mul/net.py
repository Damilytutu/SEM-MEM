...
def lstmNet(input_shape1, input_shape2):
    ...
    lstm3_t = LSTM(100, dropout=0.2, recurrent_dropout=0.2)(dp2_t)
    dp3_t = Dropout(0.5)(lstm3_t)
    out1_t = Dense(nb_classes, activation='softmax')(dp3_t)
    model1 = Model(input_t, out1_t)

    #model1.summary()

    # load weights
    model1.load_weights('shape.hdf5')
    ...
    lstm3_s = LSTM(100, dropout=0.2, recurrent_dropout=0.2)(dp2_s)
    dp3_s = Dropout(0.5)(lstm3_s)
    out1_s = Dense(nb_classes, activation='softmax')(dp3_s)
    model2 = Model(input_s, out1_s)
    #model2.summary()

    # load weights
    model2.load_weights('motion.hdf5')
